...
int main(int argc, char* argv[]) {
    ...
    double res = 0;
    for (int i=0; i<N; i++) { c[i] = (double)a[i] * (double)b[i]; res += c[i]; }
    printf("Actual: %e\n", res);

    float res_zero = 0;
    fesetround(FE_TOWARDZERO);
    for (int i=0; i<N; i++) { c_zero[i] = a[i]*b[i]; res_zero += c_zero[i]; }
    float error_zero = std::fabs(res - res_zero);
    printf("Round towards zero error: %e\n", error_zero);

    float res_down = 0;
    fesetround(FE_DOWNWARD);
    for (int i=0; i<N; i++) { c_down[i] = a[i]*b[i]; res_down += c_down[i]; }
    float error_down = std::fabs(res - res_down);
    printf("Round downward error: %e\n", error_down);

    float res_up = 0;
    fesetround(FE_UPWARD);
    for (int i=0; i<N; i++) { c_up[i] = a[i]*b[i]; res_up += c_up[i]; }
    float error_up   = std::fabs(res - res_up);
    printf("Round upward error: %e\n", error_up);

    float res_nearest = 0;
    fesetround(FE_TONEAREST);
    for (int i=0; i<N; i++) { c_nearest[i] = a[i]*b[i]; res_nearest += c_nearest[i]; }
    float error_nearest = std::fabs(res - res_nearest);
